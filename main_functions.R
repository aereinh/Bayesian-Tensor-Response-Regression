...
btrr.long_rcpp <- function(Y,Time=NULL,Visit=NULL,Ci=NULL,Xi=NULL,Zti=NULL,ID=NULL,R=rep(2,7),niter=1000,
                           null_M=F, null_Bi=F,null_Gamma=F, null_Thetai=F, null_Btm=F, null_Ds=F, null_Gammak=F, null_baseline=F,  
                           a.sig=1, b.sig=1, a.tau=1, b.tau=1,a.lam=1, b.lam=1, a.alpha=1, b.alpha=1,sigma_log_alpha=0.01,alpha.init=10, show.prog=T, prog.count=10, show.allsteps=F) {
    ...
    for (iter in 1:niter) {
      ...
      if (!null_M) {
        ...
        list[beta.new, w.new, lambda.new, tau.new, sigma2.new, alpha.new] <- btrr.1var.1visit.1iter_rcpp(Y = Y_M, xc = as.matrix(rep(1,N)), R=R[1], 
        ...
        M_iter <- c(TP.rankR(lapply(beta.store[[1]],function(x) x[,,iter+1])))
      }
      ...
      if (!null_Bi) {
        ...
        for (i in 1:Ni){
          ...
          list[beta.new, w.new, lambda.new, tau.new, sigma2.new, alpha.new] <- btrr.1var.1visit.1iter_rcpp(Y = Y_Bi, xc = as.matrix(rep(1,dim(Y_Bi)[1])), R=R[2],
      ...
      if (!null_Gamma) {
        ...
        list[beta.new, w.new, lambda.new, tau.new, sigma2.new, alpha.new] <- btrr.1var.1visit.1iter_rcpp(Y = Y_G, xc = as.matrix(Time), R=R[3], 
      ...
      if (!null_Thetai) {
        ...
        for (i in 1:Ni){
          ...
          list[beta.new, w.new, lambda.new, tau.new, sigma2.new, alpha.new] <- btrr.1var.1visit.1iter_rcpp(Y = Y_Thetai, xc = as.matrix(Time_i), R=R[4],
      ...
      if (!null_Btm) {
        ...
        for (t in ifelse(null_baseline,2,1):Nt) {
          ...
          for (m in 1:M) {
            ...
            list[beta.new, w.new, lambda.new, tau.new, sigma2.new, alpha.new] <- btrr.1var.1visit.1iter_rcpp(Y=Y_Btm, xc=as.matrix(Ci[ID_t,m]), R=R[5],
      ...
      if (!null_Ds) {
        ...
        for (s in 1:S) {
          ...
          list[beta.new, w.new, lambda.new, tau.new, sigma2.new, alpha.new] <- btrr.1var.1visit.1iter_rcpp(Y=Y_Ds, xc=as.matrix(sapply(ID, function(x) Xi[x,s])), R=R[6],
      ...
      if (!null_Gammak) {
        ...
        for (k in 1:K){
          ...
          list[beta.new, w.new, lambda.new, tau.new, sigma2.new, alpha.new] <- btrr.1var.1visit.1iter_rcpp(Y = Y_Gk, xc = as.matrix(Zti[,k]), R=R[7], 
