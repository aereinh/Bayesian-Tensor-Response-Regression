...
cppFunction(code = '
NumericVector getSums_drjn(NumericVector Y_drjn, NumericVector outer_dr) {
...
NumericVector sums_drjn(2);
sums_drjn(0)=sumSq_outer_drjn;
sums_drjn(1)=sum_Y_outer_drjn;
return sums_drjn;
}            
')

cppFunction(code = '
double getSumPow(NumericVector x, int power) {
double sumSq = 0;
for (int i=0; i<x.size(); ++i) {
  sumSq = sumSq + pow(x(i),power);
}
return sumSq;
}            
')
...
cppFunction(code = '
NumericVector index_Tensor3D_C(NumericVector X, IntegerVector dim, int margin, int index) {
int sliceLen;
if (margin==0) {
  sliceLen=dim(1)*dim(2);
} else if (margin==1) {
  sliceLen=dim(0)*dim(2);
} else if (margin==2) {
  sliceLen=dim(0)*dim(1);
}
NumericVector slice(sliceLen);

if (margin==0) {
  for (int i=0; i<sliceLen; ++i) {
    int tensor_index = index + (i%dim(1))*dim(0) + (i/dim(1))*dim(0)*dim(1);
    slice(i) = X(tensor_index);
  }
} else if (margin==1) {
  for (int i=0; i<sliceLen; ++i) {
    int tensor_index = dim(0)*index + (i%dim(0)) + (i/dim(0))*dim(0)*dim(1);
    slice(i) = X(tensor_index);
  }
} else if (margin==2) {
  for (int i=0; i<sliceLen; ++i) {
    int tensor_index = dim(0)*dim(1)*index + (i%(dim(0)*dim(1)));
    slice(i) = X(tensor_index);
  }
}
return slice;
...
cppFunction(code = '
NumericVector get_ab_drj3D(NumericMatrix Yrvec, IntegerVector dim, int margin, int index, NumericVector outer_dr, NumericVector xc, NumericVector sigma2) {
// initialize results
double a_drj = 0;
double b_drj = 0;
int N = Yrvec.nrow();
...
for (int n=0; n<N; ++n) {
  ...
  NumericVector Yrvec_n = Yrvec(n,_);
  ...
  // add to a_drj and b_drj
  a_drj = a_drj + (pow(xc(n),2)/sigma2(n))*sumSq_outer_drjn;
  b_drj = b_drj + (xc(n)/sigma2(n))*sum_Y_outer_drjn;
  
}
NumericVector ab_drj(2);
ab_drj(0) = a_drj;
ab_drj(1) = b_drj;
...
cppFunction(code = '
NumericVector get_ab_drj2D(NumericMatrix Yrvec, IntegerVector dim, int margin, int index, NumericVector outer_dr, NumericVector xc, NumericVector sigma2) {
// initialize results
double a_drj = 0;
double b_drj = 0;
int N = Yrvec.nrow();
...
for (int n=0; n<N; ++n) {
  ...
  if (margin==0) {
    for (int i=0; i<sliceLen; ++i) {
      int tensor_index = index + i*dim(0);
      Y_drjn(i) = Yrvec_n(tensor_index);
    }
  } else if (margin==1) {
    for (int i=0; i<sliceLen; ++i) {
      int tensor_index = dim(0)*index + (i%dim(0));
      Y_drjn(i) = Yrvec_n(tensor_index);
    }
  }
...
cppFunction(code = '
NumericVector TP_rankR_3D(NumericMatrix margin0, NumericMatrix margin1, NumericMatrix margin2) {
int p0 = margin0.nrow();
int p1 = margin1.nrow();
int p2 = margin2.nrow();
int tensor_len = p0*p1*p2;
int R = margin0.ncol();
NumericVector tensor_vec(tensor_len);
for (int v=0; v<tensor_len; ++v) {
  int i0 = v%p0;
  int i1 = (v/p0)%p1;
  int i2 = (v/(p0*p1))%p2;
  for (int r=0; r<R; ++r) {
    tensor_vec(v) = tensor_vec(v) + margin0(i0,r)*margin1(i1,r)*margin2(i2,r);
  }
}
return tensor_vec;
}            
...
cppFunction(code = '
NumericVector TP_rankR_2D(NumericMatrix margin0, NumericMatrix margin1) {
int p0 = margin0.nrow();
int p1 = margin1.nrow();
int tensor_len = p0*p1;
int R = margin0.ncol();
NumericVector tensor_vec(tensor_len);
for (int v=0; v<tensor_len; ++v) {
  int i0 = v%p0;
  int i1 = (v/p0)%p1;
  for (int r=0; r<R; ++r) {
    tensor_vec(v) = tensor_vec(v) + margin0(i0,r)*margin1(i1,r);
  }
}
return tensor_vec;
}            
...
cppFunction(code = '
NumericVector TP_2D(NumericVector margin0, NumericVector margin1) {
int p0 = margin0.size();
int p1 = margin1.size();
int tensor_len = p0*p1;
NumericVector tensor_vec(tensor_len);
for (int v=0; v<tensor_len; ++v) {
  int i0 = v%p0;
  int i1 = (v/p0)%p1;
  tensor_vec(v) = margin0(i0)*margin1(i1);
...
#   beta_d_prev = beta[[d]]
#   for (r in 1:R) {
#     sum_marg = sum(beta_d_prev[c(1,p[d]),r]^2)+(1+exp(-2*alpha_prev[d,r]))*sum(beta_d_prev[c(2:(p[d]-1)),r]^2)-2*exp(-alpha_prev[d,r])*sum(beta_d_prev[-1,r]*beta_d_prev[-p[d],r])
#     w_dr = w_prev[d,r]
#     chi_tau = chi_tau + sum_marg/(w_dr*(1-exp(-2*alpha_prev[d,r])))
#   }
# }
