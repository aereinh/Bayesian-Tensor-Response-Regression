...
simulate_Y <- function(p, Time=NULL, Visit=NULL, Ci=NULL, Xi=NULL, Zti=NULL, ID=NULL, Mu=NULL, Bi=NULL, Gamma=NULL, Thetai=NULL, Btm=NULL, Ds=NULL, Gammak=NULL, noise_var=0) {
...
getDeviance_alliter <- function(btrr_results, Y.train, ID, Time, Visit, Ci, Xi, Zti, show.prog=T, prog.count=10) {
  ...
  terms <- unlist(lapply(btrr_results[[1]],function(x) !is.null(x)))
...
btrr.1var.1visit.1iter_rcpp <- function(Y, xc, R=2,
  ...
  if (update.tau) {
    ...
    #tau.new <- rgig(1,a.tau-.5*R*getSumPow(p,1),chi_tau,2*b.tau)
    tau.new <- .Call("rgig",1,a.tau-.5*R*getSumPow(p,1),chi_tau,2*b.tau,PACKAGE="GIGrvg")
  }
